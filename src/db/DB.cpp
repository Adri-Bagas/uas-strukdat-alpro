...
void DB::load_endings(const std::string& file_path) {
    Utils::Logger::log("DB: Mulai memuat ending dari " + file_path);
    if (!fs::exists(file_path)) {
        Utils::Logger::log("DB ERROR: File ending " + file_path + " tidak ditemukan!");
        return;
    }
    ...
    try {
        ...
        if (j.is_object()) {
            for (auto& [key, value] : j.items()) {
                ...
                if (value.is_array()) {
                    for (const auto& line_j : value) {
                        if (line_j.is_string()) {
                            lines.push_back(line_j.get<std::string>());
                        }
                    }
                } else if (value.is_string()) {
                    lines.push_back(value.get<std::string>());
                }
                endings_db[key] = lines;
                Utils::Logger::log("DB: Terdaftar ending '" + key + "'");
            }
        }
    } catch (const std::exception& e) {
        Utils::Logger::log("DB ERROR: Gagal parse JSON di ending " + file_path + ": " + e.what());
    }
}

std::vector<std::string> DB::get_ending(const std::string& id) const {
    auto it = endings_db.find(id);
    if (it != endings_db.end()) return it->second;
    return {};
}