...
void MainPage::create_windows() {
    ...
    int col1_w = right_w * 0.35; // Increased width for side-by-side stats
...
void MainPage::draw() {
    ...
    draw_title(win_thought, "Pikiran", w_right, 2); wnoutrefresh(win_thought);
...
void MainPage::type_new_text(WINDOW* win, const char* title, int width, 
    ...
    if (available_history_lines > 0) {
        ...
        for (int i = 0; i < draw_count; ++i) mvwprintw(win, start_y + i, 2, "%s", history_all[start_idx + i].c_str());
    }
    ...
    auto redraw_all = [&]() {
        ...
        for (size_t i = 0; i < lines.size(); ++i) mvwprintw(win, start_y + i, 2, "%s", lines[i].c_str());
    ...
    for (const auto& line_str : new_lines_all) {
        ...
        if (skipped) mvwprintw(win, current_y, 2, "%s", line_str.c_str());
        else {
            int current_x = 2;
            for (char c : line_str) {
